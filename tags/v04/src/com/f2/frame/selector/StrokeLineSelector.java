...
public class StrokeLineSelector extends JFrame implements ChangeListener, ItemListener {
	...
	public void setPaintBrush(PaintBrush brush) {
        ...
        if(mPaintBrush != null) {
        	int value = mPaintBrush.getStrokeLineWidth();
        	sliderStrokeWidth.setValue(value);
    ...
    {    
		...
		mPaintBrush.setStrokeLineWidth(value);
		setTitle("笔宽 (" + value + "像素)");
		repaintComponent();
    }  
	...
	public void itemStateChanged(ItemEvent event) {
		if(event.getSource() == comboStrokeWidth) {
			if (event.getStateChange() == ItemEvent.SELECTED) {
				...
				mPaintBrush.setStrokeLineWidth(cbWidth.getStrokeWidth());
		...
		if(event.getSource() == comboStrokeDash) {
			if (event.getStateChange() == ItemEvent.SELECTED) {
				...
				mPaintBrush.setStrokeDash(cbDash.getStrokeDash());
				setVisible(false); //dispose();
				repaintComponent();
			}
		}
